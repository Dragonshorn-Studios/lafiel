...
Create an OVHcloud API application, then delegate the smallest read-only rights: `GET /me` for the connection test, and `GET` on the service endpoints the inventory adapter reads (`/service`, `/service/*`, `/order/catalog/formatted/*`). Lafiel never calls a mutating OVH endpoint — no create, renew, scale, or delete. See the [OVH rights delegation guide](https://help.ovhcloud.com/csm/de-api-api-rights-delegation?id=kb_article_view&sysparm_article=KB0068603).

## Known provider gaps (v1 checklist)

- **OVH**: adapter proven against synthetic fixtures only — the [real-account spike](https://github.com/Dragonshorn-Studios/lafiel/issues/9) has not run yet. Renewal prices come from the account-subsidiary catalog and are estimates, never actuals. Auto-renew flags without a known renewal date are not persisted. No invoice actuals, no usage metering.
