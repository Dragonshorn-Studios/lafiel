...
- **OVH**: adapter proven against synthetic fixtures shaped from the published schemas — the [real-account spike](https://github.com/Dragonshorn-Studios/lafiel/issues/9) has not run yet. Inventory is `GET /services` (ids) + `GET /services/{id}` (expanded). Quotes come from `billing.pricing` on that expanded service (one fact per id, `priceInUcents`); `/service/{id}/renew` is only a solo-strategy fallback, never a bundled order preview. The public vps/ip catalog only backfills unpriced services as a marked fallback estimate. Renewal dates come from `billing.nextBillingDate`. No invoice actuals; Public Cloud usage and billing are an explicit unsupported gap, never a known zero (#51).
