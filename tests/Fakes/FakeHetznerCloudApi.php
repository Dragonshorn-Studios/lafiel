...
/**
 * Scriptable in-memory HetznerCloudApi. Tests queue response bodies
 ...
 * implements HetznerCloudApi, only GET is expressible: the same
 * read-only guarantee the real client carries.
 */
final class FakeHetznerCloudApi implements HetznerCloudApi
{
    ...
    public function get(string $path, array $query = []): array
    {
        ...
        if (! array_key_exists($path, $this->responses)) {
            throw new RuntimeException("FakeHetznerCloudApi has no scripted response for [{$path}].");
        }
