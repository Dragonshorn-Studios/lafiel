...
/**
 * Scriptable in-memory ContaboApi. Tests queue response bodies (or
 * exceptions) per path — a response may be a callable receiving the
 * query parameters, which is how multi-page collections are scripted —
 * and can inspect every call afterwards. Because it implements
 * ContaboApi, only GET is expressible: the same read-only guarantee
 * the real client carries, token exchange included in the real client.
 */
final class FakeContaboApi implements ContaboApi
{
    /** @var array<string, mixed> bodies or callables(array): array */
    ...
    public function get(string $path, array $query = []): array
    {
        ...
        if (! array_key_exists($path, $this->responses)) {
            throw new RuntimeException("FakeContaboApi has no scripted response for [{$path}].");
        }
