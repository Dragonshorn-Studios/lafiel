...
new #[Title('Provider settings')] class extends Component {
...
<section class="w-full">
    ...
    <flux:heading level="2" class="sr-only">{{ __('Provider settings') }}</flux:heading>

    <x-pages::settings.layout
        :heading="__('Providers')"
        :subheading="__('Connect the provider accounts Lafiel should watch')"
    >
        <div class="my-6 w-full space-y-6">
            <form wire:submit="{{ $editingAccountId === null ? 'connect' : 'update' }}" class="space-y-6">
                <flux:heading size="lg">
                    {{ $editingAccountId === null ? __('Connect an OVH account') : __('Replace OVH credentials') }}
                </flux:heading>
            ...
            <flux:card>
                ...
                @forelse ($this->accounts as $account)
                    ...
                    <div class="mt-4 space-y-3 border-b border-zinc-200 pb-4 last:border-b-0 dark:border-zinc-700" data-test="provider-account">
                        ...
                        @if ($check !== null)
                            @if ($check['status'] === ConnectionStatus::Connected->value)
                                <flux:icon.check-circle variant="mini" class="inline h-4 w-4 text-green-600" />
                            @elseif ($check['status'] === ConnectionStatus::Rejected->value)
                                <flux:icon.exclamation-triangle variant="mini" class="inline h-4 w-4 text-red-600" />
                            @else
                                <flux:icon.clock variant="mini" class="inline h-4 w-4 text-amber-500" />
                            @endif

                            <span class="text-sm" data-test="connection-check">{{ $check['message'] }}</span>
                        @endif
                ...
                @empty
                    <p class="mt-3 text-sm text-zinc-500 dark:text-zinc-400">{{ __('No provider accounts yet.') }}</p>
                @endforelse
            </flux:card>
            ...
            <flux:card>
                ...
                <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
                    <a href="https://help.ovhcloud.com/csm/de-api-api-rights-delegation?id=kb_article_view&sysparm_article=KB0068603" target="_blank" rel="noopener noreferrer" class="font-medium text-zinc-900 underline decoration-zinc-300 underline-offset-4 hover:decoration-zinc-400 dark:text-white">{{ __('How to delegate OVH API rights') }}</a>
                </p>
            </flux:card>
        </div>
    </x-pages::settings.layout>
